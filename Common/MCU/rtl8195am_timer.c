...
extern IRQ_FUN Timer2To7VectorTable[ MAX_TIMER_VECTOR_TABLE_NUM ]; /* 0x10000358 */
...
void HalTimerIrq2To7Handle_Patch( IN void *Data )
{
  ...
  for ( CheckIndex = 2; CheckIndex < 8; CheckIndex++ )
  {
      ...
      && ( HAL_TIMER_READ32(TIMER_INTERVAL*CheckIndex + TIMER_CTL_REG_OFF) & BIT0 ) )
    {
      ...
      //3 Clear Timer ISR
      USED( HAL_TIMER_READ32( TIMER_INTERVAL*CheckIndex + TIMER_EOI_OFF ) );
    }
  }
}

HAL_STATUS HalTimerIrqRegisterRtl8195a_Patch( IN void *Data )
{
  ...
  else
  {
    if ( pHalTimerAdap->TimerId > 1 )
    {
      ...
      Timer2To7HandlerData[ pHalTimerAdap->TimerId - 2 ] = (uint32_t) pHalTimerAdap->IrqHandle.Data;
    }
    else
    {
...
{ 
   ...
   */
  HAL_TIMER_WRITE32( (TIMER_INTERVAL*pHalTimerAdap->TimerId + TIMER_CTL_REG_OFF), ControlReg );
  ...
  { 
    ...
    HAL_TIMER_WRITE32( (TIMER_INTERVAL*pHalTimerAdap->TimerId + TIMER_LOAD_COUNT_OFF), 0xFFFFFFFF );
  }
...
#elif ( CONFIG_CHIP_X_CUT == CONFIG_CHIP_C_CUT )
// Patch for C Cut
HAL_STATUS
...
HAL_STATUS HalTimerIrqUnRegisterRtl8195a_Patch( IN void *Data )
{ 
  PTIMER_ADAPTER pHalTimerAdap = (PTIMER_ADAPTER) Data;
...
#if ( CONFIG_CHIP_X_CUT < CONFIG_CHIP_D_CUT )
...
//    DBG_TIMER_INFO("%s: Load Count=0x%x\r\n", __FUNCTION__, LoadCount);
  // set TimerLoadCount Register
  HAL_TIMER_WRITE32( (TIMER_INTERVAL*TimerId + TIMER_LOAD_COUNT_OFF), LoadCount );
}
...
{
  USED( HAL_TIMER_READ32( TIMER_INTERVAL*TimerId + TIMER_EOI_OFF ) );
}
