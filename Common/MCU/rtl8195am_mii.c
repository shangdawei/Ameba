...
u16 HalMiiRwPhyRegRtl8195a( u8 Operation, u8 Address, u16 Data )
{
  u32 tmp;

  tmp = ( Operation << 31 ) | ( ( PHY_ADDRESS << 26 ) & MIIA_PHY_ADDR_MASK )
    | ( ( Address << 16 ) & MIIA_PHY_REG_ADDR_MASK ) | Data;
...
s32 HalMiiInitRtl8195a( IN VOID )
{
  ...
  /* Tx settings */
  HAL_MII_WRITE32( REG_MII_TC,
    ( ( TC_IFG_TIME << 10 ) & TC_IFG_MASK ) | ( ( TC_NORMAL_MODE << 8 ) & TC_LBK_MASK ) );// Normal mode

  /* Rx settings */
  HalMiiGetMacAddressRtl8195a( macAddr );
  HAL_MII_WRITE32( REG_MII_IDR0,
    ( macAddr[ 0 ] << 24 ) | ( macAddr[ 1 ] << 16 ) | ( macAddr[ 2 ] << 8 ) | macAddr[ 3 ] );
  ...
  HAL_MII_WRITE32( REG_MII_IOCMD1,
    ( ( IOCMD1_DSC_FMT_EXTRA << 28 ) & IOCMD1_DSCFMTEXTRA_MASK ) | IOCMD1_EN_1GB );// Extra desc. format = 011, support 1GB addressing
  HAL_MII_WRITE32( REG_MII_IOCMD,
    IOCMD_SHORT_DES_FMT | ( ( IOCMD_TXFTH_256 << 19 ) & IOCMD_TXFTH_MASK )
      | ( ( IOCMD_RXFTH_256 << 11 ) & IOCMD_RXFTH_MASK ) | IOCMD_TE );// short desc. format = 1, Tx & Rx FIFO threshold = 256 bytes, enable MII Tx
  ...
  for ( i = 0; i < ( pEthAdapter->tx_desc_num ); i++ )
  {
    ...
    TxDesc[ i ].dw2 = ( ( TX_DESC_VLAN_REMOVE << 25 ) & TX_DESC_VLAN_ACT_MASK )
      | ( C_VLAN_HDR & TX_DESC_VLAN_TAG_MASK );
    TxDesc[ i ].dw3 = 0;
    TxDesc[ i ].dw4 = 0;
  }
  /* Fill Rx descriptors */
...
VOID HalMiiDeInitRtl8195a( IN VOID )
{
#if 0
  if (isGdmaAllocated)
...
u32 HalMiiSendPacketRtl8195a( IN VOID )
{
...
u32 HalMiiReceivePacketRtl8195a( IN VOID )
{
...
u32 HalMiiGetLinkStatusRtl8195a( IN VOID )
{
  u32 ret;
