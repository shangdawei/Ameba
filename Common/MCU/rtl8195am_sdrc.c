...
DRAM_INFO SdrDramDev = {
  DRAM_INFO_TYPE, DRAM_INFO_COL_ADDR_WTH, DRAM_INFO_BANK_SZ, DRAM_INFO_DQ_WTH };

HAL_CUT_B_RAM_DATA_SECTION
DRAM_MODE_REG_INFO SdrDramModeReg = {
  BST_LEN_4, SENQUENTIAL, 0x3, // Mode0Cas: 3
  0x0, // Mode0Wr
  0,   // Mode1DllEnN
  0,   // Mode1AllLat
  0    // Mode2Cwl
...
DRAM_DEVICE_INFO SdrDramInfo = {
  &SdrDramDev, &SdrDramModeReg, &SdrDramTiming, DRAM_TIMING_TCK, DFI_RATIO_1 };
...
)
{
  ...
  else if ((DramInfo->Dev->DeviceType) == DRAM_DDR_3)
  {
    ...
    if (DramInfo->ModeReg->Mode1AllLat == 1)
    { // CL-1
      AddLat = CasRd -1;
    }
    else if (DramInfo->ModeReg->Mode1AllLat == 2)
    { // CL-2
      AddLat = CasRd -2;
    }
    else
  ...
  if ((DramInfo->Dev->DeviceType) != SDR)
  { // DDR2/3
