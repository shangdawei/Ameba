...
//        }
    ...
    if ( ( ( pHalSsiAdapter->InterruptMask & BIT_IMR_TXEIM ) != 0 )
...
u32 _SsiIrqHandleRtl8195a( VOID *Adaptor )
{
  ...
  if ( ( InterruptStatus & BIT_ISR_TXEIS )
    || ( ( ( pHalSsiAdaptor->InterruptMask & BIT_IMR_TXEIM ) != 0 )
      && ( pHalSsiAdaptor->TxLength > 0 ) ) )
  {
...
HAL_STATUS HalSsiInitRtl8195a_Patch( VOID *Adaptor )
{
  ...
  if ( Role & SSI_MASTER )
  {
    if ( ( TransferMode == TMOD_RO ) || ( TransferMode == TMOD_EEPROM_R )
      || ( ( DataFrameFormat == FRF_NS_MICROWIRE )
        && ( MicrowireTransferMode == MW_TMOD_SEQUENTIAL ) ) )
    {
...
HAL_STATUS HalSsiPinmuxEnableRtl8195a_Patch( VOID *Adaptor )
{
  ...
  PERI_ON_BASE + REG_SPI_MUX_CTRL, HAL_READ32( PERI_ON_BASE, REG_SPI_MUX_CTRL ) );
...
HAL_STATUS HalSsiStopRecvRtl8195a( VOID *Data )
{
  ...
  if ( NULL != pHalGdmaAdapter )
  {
    ...
    if ( ( NULL != pHalGdmaOp ) && ( HalGdmaQueryChEnRtl8195a( (VOID*) pHalGdmaAdapter ) ) )
    {
      ...
      while ( ( HAL_GDMAX_READ32( pHalGdmaAdapter->GdmaIndex, REG_GDMA_CH_EN )
        & ( pHalGdmaAdapter->ChEn ) ) != 0 )
      {
...
const DW_SSI_DEFAULT_SETTING SpiDefaultSetting = {
  .RxCompCallback = NULL, .RxCompCbPara = NULL, .RxData = NULL, .TxCompCallback = NULL,
  .TxCompCbPara = NULL, .TxData = NULL,
  ...
  .InterruptPriority = 10, .RxLength = 0, .RxLengthRemainder = 0,
  ...
  .DataFrameNumber = 0, .ControlFrameSize = CFS_1_BIT, .DataFrameFormat = FRF_MOTOROLA_SPI,
  .DataFrameSize = DFS_8_BITS, .DmaControl = 0, // default DMA is disable
  .InterruptMask = 0x0, .MicrowireDirection = MW_DIRECTION_MASTER_TO_SLAVE, .MicrowireHandshaking =
    MW_HANDSHAKE_DISABLE, .MicrowireTransferMode = MW_TMOD_NONSEQUENTIAL, .SclkPhase =
    SCPH_TOGGLES_AT_START, .SclkPolarity = SCPOL_INACTIVE_IS_HIGH, .SlaveOutputEnable =
    SLV_TXD_ENABLE,  // Slave
  .TransferMode = TMOD_TR, .TransferMechanism = SSI_DTM_INTERRUPT };
