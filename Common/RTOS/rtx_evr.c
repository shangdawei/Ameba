...
// http://www.keil.com/pack/doc/compiler/EventRecorder/html/EventRecorder.html

/* This section describes how the Event Recorder collects event data, generates time stamps,
 * and transfers this information via a debug unit to a host computer.
 * The Event Recorder is implemented in the target application using the software component :
 * Compiler:Event Recorder which adds the source file EventRecorder.c to the application.
 * Each event is stored in a 16-byte structure that is composed of a 16-bit id, 32-bit time stamp,
 * two 32-bit data values and consistency check values.
 * To store these events, a circular buffer is provided that can store a minimum of 8 events.
 * The size of this circular buffer is configurable with the #define EVENT_RECORD_COUNT.
 * The file EventRecorderConf.h is used to define the configuration parameters of the Event Recorder.
 * This file must be part of every project that is using the Event Recorder.
 * The configuration settings are explained below:
 *
 * Number of Records
 * EVENT_RECORD_COUNT
 * Specifies the number or records stored in the Event Record Buffer. Each record is 16 bytes.
 *
 * Time Stamp Source
 * EVENT_TIMESTAMP_SOURCE
 * Specifies timer that is used as time base. Default is the DWT Cycle Counter that is part of that CoreSight debug interface.
 * The DWT Cycle Counter is not available in Cortex-M0 and Cortex-M0+.
 *
 * <0=> DWT Cycle Counter
 * <1=> SysTick
 * <2=> CMSIS RTOS
 ...
 * If the SysTick timer is used as time stamp source the following setting are relevant.
 *
 *   SysTick Input Clock Frequency [Hz]
 *   SYSTICK_CLOCK
 *   Defines SysTick input clock that is typical identical with processor clock.
 *
 *   SysTick Interrupt Frequency [us]
 *   SYSTICK_PERIOD_US
